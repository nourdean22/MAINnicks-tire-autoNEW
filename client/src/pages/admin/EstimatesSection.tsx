  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-card border border-border/30 p-5">
          <div className="flex items-start justify-between mb-3">
            <span className="font-mono text-[10px] text-foreground/50 tracking-wider uppercase">Total Estimates</span>
            <DollarSign className="w-4 h-4 text-foreground/30" />
          </div>
          <div className="font-heading font-bold text-3xl text-foreground tracking-tight">{estimateLeads.length}</div>
          <p className="font-mono text-[10px] text-foreground/40 mt-1">Customers researching repair costs</p>
        </div>
        <div className="bg-card border border-border/30 p-5">
          <div className="flex items-start justify-between mb-3">
            <span className="font-mono text-[10px] text-foreground/50 tracking-wider uppercase">Conversion Potential</span>
            <TrendingUp className="w-4 h-4 text-foreground/30" />
          </div>
          <div className="font-heading font-bold text-3xl text-primary tracking-tight">HIGH</div>
          <p className="font-mono text-[10px] text-foreground/40 mt-1">Estimate users are 3x more likely to book</p>
        </div>
        <div className="bg-card border border-border/30 p-5">
          <div className="flex items-start justify-between mb-3">
            <span className="font-mono text-[10px] text-foreground/50 tracking-wider uppercase">Avg Response Time</span>
            <Clock className="w-4 h-4 text-foreground/30" />
          </div>
          <div className="font-heading font-bold text-3xl text-emerald-400 tracking-tight">&lt;5s</div>
      ...
      <div className="bg-card border border-border/30 p-6">
        <h3 className="font-heading font-bold text-sm text-foreground tracking-wider uppercase mb-4">RECENT ESTIMATE REQUESTS</h3>
        
        {estimateLeads.length === 0 ? (
          <div className="text-center py-12">
            <Search className="w-8 h-8 text-foreground/20 mx-auto mb-3" />
            <p className="text-foreground/40 font-mono text-xs">No estimate requests yet</p>
            <p className="text-foreground/30 font-mono text-[10px] mt-1">When customers use the AI Estimator, their requests appear here</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-border/30">
                  ...
                  <th className="font-mono text-[10px] text-foreground/40 tracking-wider uppercase py-2 pr-4">Customer</th>
                  <th className="font-mono text-[10px] text-foreground/40 tracking-wider uppercase py-2 pr-4">Vehicle</th>
                  <th className="font-mono text-[10px] text-foreground/40 tracking-wider uppercase py-2 pr-4">Repair</th>
              ...
              <tbody>
                {estimateLeads.map((lead) => (
                  <tr key={lead.id} className="border-b border-border/10">
                    ...
                    <td className="py-2.5 pr-4">
                      <span className="font-mono text-xs text-foreground">{lead.name || "Anonymous"}</span>
                    </td>
                    <td className="py-2.5 pr-4">
                      <div className="flex items-center gap-1.5">
                        <Car className="w-3 h-3 text-foreground/30" />
                        <span className="font-mono text-xs text-foreground/70">{lead.vehicle || "—"}</span>
                      </div>
                    </td>
                    <td className="py-2.5 pr-4">
                      <span className="font-mono text-xs text-foreground/70 line-clamp-1">{lead.problem || "—"}</span>
                    </td>
                    <td className="py-2.5">
