  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h2 className="font-bold text-xl text-foreground tracking-wider">ADVANCED JOB BOARD</h2>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-1.5 text-[12px] tracking-wider ${viewMode === "kanban" ? "bg-primary text-primary-foreground" : "bg-card border border-border/30 text-foreground/60"}`}
          >
            KANBAN
          </button>
          <button
            onClick={() => setViewMode("list")}
            className={`px-3 py-1.5 text-[12px] tracking-wider ${viewMode === "list" ? "bg-primary text-primary-foreground" : "bg-card border border-border/30 text-foreground/60"}`}
          >
            LIST
          </button>
        </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select
          ...
          className="bg-card border border-border/30 px-3 py-2 text-[12px] text-foreground"
        >
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="font-bold text-foreground text-sm">{job.name}</span>
                      <span className="text-[12px] text-foreground/40">{job.phone}</span>
                    ...
                    <p className="text-[12px] text-foreground/50">{job.vehicle || "No vehicle"} — {job.service}</p>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setAssigningBookingId(null)}>
          <div className="bg-card border border-border/30 p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-foreground tracking-wider">ASSIGN TECHNICIAN</h3>
            ...
            <div className="space-y-2">
                ...
                return (
                  ...
                  >
                    <div className="w-8 h-8 bg-primary/20 flex items-center justify-center flex-shrink-0">
                      <span className="font-bold text-sm text-primary">{tech.name.charAt(0)}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <span className="font-bold text-sm text-foreground block">{tech.name}</span>
              ...
              {(!techList || techList.length === 0) && (
                <p className="text-center py-4 text-[12px] text-foreground/40">
  ...
  return (
    <div className={`bg-card border border-border/20 p-3 hover:border-border/40 transition-colors ${getPriorityColor(job.urgency, stage)}`}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <span className="font-bold text-xs text-foreground leading-tight">{job.name}</span>
        {job.urgency === "emergency" && (
      ...
      <div className="flex items-center gap-1 mt-2 pt-2 border-t border-border/10">
        ...
        {stageIndex < STAGES.length - 1 && (
          <button
            ...
            className="ml-auto flex items-center gap-1 px-2 py-0.5 bg-primary/10 text-primary text-[9px] tracking-wider hover:bg-primary/20 transition-colors"
          >
            NEXT <ArrowRight className="w-2.5 h-2.5" />
          </button>
        )}
        {stageIndex === STAGES.length - 1 && (
          <span className="ml-auto text-[9px] text-emerald-400 tracking-wider">DONE</span>
        )}
      </div>
    </div>
  );
