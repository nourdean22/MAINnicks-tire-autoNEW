...
function CityNavbar({ city }: { city: CityData }) {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-nick-dark/90 backdrop-blur-md shadow-lg" : "bg-transparent"}`} aria-label="City page navigation">
    ...
    <PageLayout>
      ...
      <CityNavbar city={city} />
                ...
                <div className="bg-card/50 border border-border/50 rounded-lg p-8">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-nick-yellow/20 rounded-full flex items-center justify-center">
                      <span className="font-semibold font-bold text-nick-yellow text-sm">{city.testimonial.author.charAt(0)}</span>
                    </div>
                    <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                  {topServices.map((s) => (
                    <Link key={s.slug} href={`/${s.slug}`} className="bg-card/80 border border-border/50 rounded-lg p-4 text-center hover:border-nick-yellow/30 hover:bg-card transition-colors group">
                      <span className="font-semibold font-bold text-sm text-foreground/80 group-hover:text-nick-yellow transition-colors tracking-wider">{s.title}</span>
                    </Link>
                  ))}
                </div>
              </div>
        ...
        <section className="py-12 lg:py-16 section-dark border-t border-border/50">
          <div className="container">
            <h3 className="font-semibold font-bold text-lg text-foreground/60 tracking-wider mb-6">ALSO SERVING</h3>
            <div className="flex flex-wrap gap-3">
