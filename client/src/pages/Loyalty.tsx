...
function LoyaltyDashboard() {
  ...
  if (summaryLoading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-foreground/60 text-[13px]">Loading your rewards...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      {/* Points Balance */}
      <div className="bg-[oklch(0.08_0.004_260/0.8)] border border-[oklch(0.17_0.004_260)] rounded-2xl p-6 lg:p-8 text-center">
        <span className="font-mono text-nick-teal text-xs tracking-wide">Your Balance</span>
        <div className="mt-3 flex items-center justify-center gap-3">
          <Star className="w-8 h-8 text-primary" />
          <span className="font-bold text-5xl lg:text-6xl text-primary">{points.toLocaleString()}</span>
      ...
      {rewards && rewards.length > 0 && (
        <div>
          <h3 className="font-bold text-xl text-foreground tracking-[-0.01em] mb-4">AVAILABLE REWARDS</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {rewards.map((reward: any) => {
              const canRedeem = points >= reward.pointsCost;
              return (
                <div key={reward.id} className={`bg-[oklch(0.08_0.004_260/0.8)] border border-[oklch(0.17_0.004_260)] rounded-2xl p-5 border ${canRedeem ? "border-nick-teal/30" : "border-border/20 opacity-60"}`}>
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h4 className="font-bold text-foreground text-sm tracking-wider">{reward.title}</h4>
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <span className="text-[13px] text-primary font-bold">{reward.pointsCost} pts</span>
                    <button
                      onClick={() => {
                      ...
                      className={`px-4 py-2 rounded-md font-bold text-xs tracking-wide transition-colors ${
                        canRedeem
      ...
      {transactions && transactions.length > 0 && (
        <div>
          <h3 className="font-bold text-xl text-foreground tracking-[-0.01em] mb-4">RECENT ACTIVITY</h3>
          <div className="bg-[oklch(0.08_0.004_260/0.8)] border border-[oklch(0.17_0.004_260)] rounded-2xl overflow-hidden">
            {transactions.slice(0, 10).map((tx: any, i: number) => (
              <div key={tx.id} className={`flex items-center justify-between px-5 py-3.5 ${i > 0 ? "border-t border-border/20" : ""}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${tx.points > 0 ? "bg-nick-teal/20 text-nick-teal" : "bg-primary/20 text-primary"}`}>
                ...
                <span className={`font-bold text-sm ${tx.points > 0 ? "text-nick-teal" : "text-primary"}`}>
    ...
    <PageLayout activeHref="/rewards" showChat={true}>
      ...
      <section className="bg-[oklch(0.065_0.004_260)] pt-28 pb-16 lg:pt-36 lg:pb-20">
        <div className="container max-w-4xl text-center">
          <span className="font-mono text-nick-teal text-sm tracking-wide">Loyalty Rewards</span>
          <h1 className="font-bold text-4xl lg:text-6xl text-foreground mt-3 tracking-tight">
            EARN <span className="text-primary">REWARDS</span> ON EVERY SERVICE
          </h1>
        ...
        <div className="container max-w-3xl">
          {loading ? (
            ...
            <div className="bg-[oklch(0.08_0.004_260/0.8)] border border-[oklch(0.17_0.004_260)] rounded-2xl p-8 text-center">
              <Trophy className="w-16 h-16 text-primary/60 mx-auto mb-4" />
              ...
              <a
                href={getLoginUrl()}
                className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-8 py-4 rounded-md font-bold text-sm tracking-wide hover:opacity-90 transition-colors"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              <div key={b.title} className="text-center">
                ...
                <h3 className="font-bold text-sm text-foreground tracking-[-0.01em] mb-1">{b.title}</h3>
