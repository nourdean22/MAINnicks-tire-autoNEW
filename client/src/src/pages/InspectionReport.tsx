...
const CONDITION_CONFIG = {
  ...
  yellow: { label: "Monitor", color: "text-primary", bg: "bg-primary/20", border: "border-primary/30", icon: <AlertTriangle className="w-5 h-5" /> },
...
const OVERALL_CONFIG = {
  ...
  fair: { label: "Fair — Some Items Need Attention", color: "text-primary", bg: "bg-primary/10", border: "border-primary/30" },
...
export default function InspectionReport() {
  ...
  if (isLoading) {
    return (
      <PageLayout>
        ...
        <section className="bg-[oklch(0.065_0.004_260)] pt-28 pb-20">
          <div className="container max-w-3xl text-center py-20">
            <div className="w-10 h-10 border-2 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-4" />
            <p className="text-foreground/60 text-[13px]">Loading inspection report...</p>
          </div>
        </section>
      </PageLayout>
    );
      ...
      <PageLayout>
        ...
        <section className="bg-[oklch(0.065_0.004_260)] pt-28 pb-20">
          <div className="container max-w-3xl text-center py-20">
            ...
            <h1 className="font-bold text-3xl text-foreground mb-3">Report Not Found</h1>
            ...
            <a href={BUSINESS.phone.href} className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-6 py-3 rounded-md font-bold text-sm tracking-wide">
  ...
  return (
    <PageLayout>
      ...
      {/* Header */}
      <section className="bg-[oklch(0.065_0.004_260)] pt-28 pb-12 lg:pt-36 lg:pb-16">
        <div className="container max-w-3xl">
          <div className="text-center mb-8">
            <span className="font-mono text-nick-teal text-sm tracking-wide">Digital Inspection Report</span>
            <h1 className="font-bold text-3xl lg:text-4xl text-foreground mt-3 tracking-tight">
              {inspection.vehicleInfo}
            </h1>
            {inspection.mileage && (
              <p className="text-foreground/60 text-[13px] mt-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            <div className="bg-card/50 rounded-md p-3 text-center">
              ...
              <span className="text-[12px] text-foreground/40 block">Customer</span>
              <span className="text-[13px] text-foreground">{inspection.customerName}</span>
            </div>
            <div className="bg-card/50 rounded-md p-3 text-center">
              <Wrench className="w-4 h-4 text-nick-teal mx-auto mb-1" />
              <span className="text-[12px] text-foreground/40 block">Technician</span>
              <span className="text-[13px] text-foreground">{inspection.technicianName}</span>
            </div>
            <div className="bg-card/50 rounded-md p-3 text-center">
              <Calendar className="w-4 h-4 text-nick-teal mx-auto mb-1" />
              <span className="text-[12px] text-foreground/40 block">Date</span>
              <span className="text-[13px] text-foreground">
                {new Date(inspection.createdAt).toLocaleDateString()}
              </span>
            </div>
            <div className={`rounded-md p-3 text-center ${overall.bg} border ${overall.border}`}>
              <span className="text-[12px] text-foreground/40 block mb-1">Overall</span>
              <span className={`font-bold text-sm ${overall.color}`}>{overall.label}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-8">
            <div className="bg-nick-teal/10 border border-nick-teal/20 rounded-md p-4 text-center">
              <span className="font-bold text-3xl text-nick-teal">{summary.green}</span>
              <span className="block text-[12px] text-nick-teal/80 mt-1">Good</span>
            </div>
            <div className="bg-primary/10 border border-primary/20 rounded-md p-4 text-center">
              <span className="font-bold text-3xl text-primary">{summary.yellow}</span>
              <span className="block text-[12px] text-primary/80 mt-1">Monitor</span>
            </div>
            <div className="bg-red-400/10 border border-red-400/20 rounded-md p-4 text-center">
              <span className="font-bold text-3xl text-red-400">{summary.red}</span>
              <span className="block text-[12px] text-red-400/80 mt-1">Needs Repair</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[oklch(0.055_0.004_260)] py-12 lg:py-16">
        <div className="container max-w-3xl space-y-4">
          {inspection.items?.map((item: any) => {
            ...
            return (
              <div key={item.id} className={`bg-[oklch(0.08_0.004_260/0.8)] border border-[oklch(0.17_0.004_260)] rounded-2xl p-5 border-l-4 ${cond.border}`}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-foreground tracking-wider text-sm">{item.component}</h3>
                  ...
                  <span className={`text-[12px] font-bold px-2 py-1 rounded ${cond.bg} ${cond.color}`}>
      ...
      <section className="bg-[oklch(0.065_0.004_260)] py-12 lg:py-16">
        <div className="container max-w-3xl">
          {inspection.summaryNotes && (
            <div className="bg-[oklch(0.08_0.004_260/0.8)] border border-[oklch(0.17_0.004_260)] rounded-2xl p-6 mb-8">
              <h3 className="font-bold text-foreground tracking-wider text-sm mb-3">TECHNICIAN NOTES</h3>
          ...
          <p className="text-center text-foreground/40 text-[12px] mt-6">
